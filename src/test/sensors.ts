...
describe("Movement Sensor", () => {
    describe("when receiving an event", () => {
        ...
        let queue: string[]
        before(() => {
            const handler = sensorFactory(voidHandler, 10, initialQueue)
            queue = handler("")
        })
    ...
    describe("when receiving more events than the queue depth", () => {
        let queue: string[]
        before(() => {
            const handler = sensorFactory(voidHandler, 10, hoverQueue)
            queue = handler("")
        })
    ...
    describe("when receiving events with movement", () => {
        let state: FlightState
        const stateHandler = (val: FlightState) => state = val
        const handler = sensorFactory(stateHandler, 10)
